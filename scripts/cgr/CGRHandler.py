...
from Tesi.scripts.cgr import CGRepresentation
...
class CGRHandler:
    ...
    def filter_sequence(self, sequence):
        ...
        chars = ["T", "Y", "N", "R", "M", "S", "W", "K", "D", "V", "B", "H", "P", "O"]
        ...
        if any(x in sequence for x in chars):
            ...
            for i in range(0, char_count):
                if filtered_sequence.find("T") != -1:
                    filtered_sequence = filtered_sequence.replace("T", "U")
    ...
    def count_char(self, sequence):
        ...
        char_count += tmp_sequence.count("T")
    ...
    '''
    Metodo per leggere un unico file .fa e prendere le sequenze contenute in esso.
    '''
    def read_file(self):
        self.init_dirs()
        ...
        '''
        Metodo usato per comparare la similarità tra due sequenze.
        '''
        ...
        self.generate_dataset_from_list(similar_sequences)

    '''
    Metodo per generare il Dataset a partire da una lista.
    '''
    def generate_dataset_from_list(self, sequence_list):
        for i, sequence in enumerate(sequence_list):
            counter = i + 1
            print(f"L'elemento {counter} è: {sequence}")
            bio_sequence = self.filter_sequence(sequence.upper())
            print("SEQUENZA UTILIZZATA: " + bio_sequence)
            print("COUNTER: " + str(counter))
            drawer = CGRepresentation.CGR(bio_sequence, self.CGR_type, self.outer_representation, self.rna_2structure)
            drawer.representation()
